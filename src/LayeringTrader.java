...
public class LayeringTrader extends Trader {

	static final int TRADER_TYPE = 7;
	...
	int   spoofingMultiple  = 5;									// The spoofing share = multiple * genuine share
	...
	int   lastGenuineSize = 1; // middle variable for saving genuine size

	LayeringTrader(int accountID, String symbols[]){
	...
	void generateBuyOrders(String symbol, int curTime, LinkedList <Order> ordList) 	{
		...
		if (first==0) {
			...
			int id = generateLayeringSellOrders(symbol,curTime,ordList,0);
	...
	void generateSellOrders(String symbol, int curTime, LinkedList <Order> ordList) 	{
		...
		if (first==0) {
			...
			int id = generateLayeringBuyOrders(symbol,curTime,ordList,0);
