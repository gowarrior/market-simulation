...
; TWAP market time weighted average price
; VWAP market volume weighted average price
; TransactionNum

extensions [array table]


Globals [minVolumeQueue minVolume crashPrice crash crashTriggered price buyQueue sellQueue orderNumber tradeWipeQueue tradeMatch currentAsk currentBid 
        numberBid numberAsk volume volatility  currentPriceVol totalVolume TWAP VWAP TransactionNum currentVol 
        movingAverage currentMA stopMarketMakers stopLoss movingAverageV currentMAV currentBQD 
        ...
        tblBidDepth tblAskDepth nBidDepth1 nBidDepth3 nAskDepth1 nAskDepth3]
...
; tradeQ2 - Current Quantity trading in the Open Order Book, needed for audit trail
...
to setup
  ...
  set arrayQ array:from-list n-values 10000 [0]
...
to setup-economy  ;; turtles procedure for setup
  ...
  set tblBidDepth table:make 
  set tblAskDepth table:make
...
;       set sellQueue remove tradeOrderForm sellQueue
      ...
      if(ticks > 15000)[
      if (tradeStatus = "Cancel" and tradeQuantity != 0)[
           generate-depths
           writetofile sharesOwned
      ]]
      set countticks 0
...
;        set sellQueue lput tradeOrderForm sellQueue
      ...
      if (ticks > 10000 ) [
        if (MarketPause = False) [ ;; always
          set marketPauseTickCount 0
          ;;transaction          
        ]
        if (MarketPause = true) [
          set marketPauseTickCount (marketPauseTickCount + 1)
          if(marketPauseTickCount >= 20)[
            set marketPauseTickCount 0
            set MarketPause False
            ;set stopTriggered 0
          ]
...
;  ]   ;end ask turtle
  ...
  if(totalVolume > 0 or volume > 0)[
    if(volume > 0)[
      set TransactionNum TransactionNum + 1
    ]
    set VWAP (VWAP * totalVolume + (1000 + (price / 5)) * volume) / (totalVolume + volume)
    set TWAP (TWAP * (TransactionNum - 1) + (1000 + (price / 5))) / TransactionNum
  ]
      ...
      loop  [
        ...
        if (traderListNumber = traderNumber) [
          ...
          if (tradeStatus != "Neutral" and AuditTrail = true)[
            ...
            file-type ", "
            file-type last priceReturns
            file-type ", "
            file-type nBidDepth1 
            file-type ", "
            file-type nBidDepth3
            file-type ", "  
            file-type nAskDepth1 
            file-type ", "  
            file-type nAskDepth3
            file-type ", "
...
to fundBuyerStrategy
  ...
  if(stopLoss) [
    ...
    [
      ...
      set tradePrice (currentBid - random 5)
      set tradeQuantity (min list tradeQuantity sharesOwned)
    ]
    [
      set tradeStatus "Neutral"
    ]
...
to fundSellerStrategy
  ...
  if(crash and (not crashTriggered)) [
      set tradeStatus "Sell"
      set tradeQuantity min(list 99 (round (sellPercent * minVolume)))
      set tradePrice (currentBid - 100)
      show tradeQuantity
      set crashTriggered TRUE
    ]
...
to strategyOpportunistic
  ...
  if(sharesOwned > 0 and stopLoss) [
    ; withdraw from market
    set tradeStatus "Sell"
    set tradePrice (currentBid - random 3)
    set tradeQuantity (min list tradeQuantity sharesOwned)
  ]
...
to strategyMedium
  ...
  if(stopMarketMakers) [
    ifelse(sharesOwned > 0)[
      ; withdraw from market
      set tradeStatus "Sell"
      set tradePrice (currentBid - random 10)
      set tradeQuantity (min list tradeQuantity sharesOwned)
    ]
    [
      ifelse(not stopLoss)[
        set tradeStatus "Sell"
        set tradePrice price - random 10 - 20
        ;set tradeQuantity 10
      ]
      [
        set tradeStatus "Buy"
        set tradePrice price - random 5
      ]
    ]
  ]
end
...
to strategyHigh
  ...
  if(marketPause)[
    set tradeStatus "Buy"
    set tradePrice crashPrice + random 5
  ]
  if(crashPrice > 0 and not crash)[
    ;set tradeStatus "Buy"
    if(tradeStatus = "Buy") [
      set tradePrice crashPrice + random 10 + 30
      ;set tradeQuantity 50
    ]
...
to strategyHuman
  ...
  if(humanAction = 1)[
    ...
    set tradePrice (BuyPriceTicks - 1000) * 5
    show tradePrice
  ]
  if(humanAction = 2)[
    set humanAction 0
    ...
    set tradePrice (SellPriceTicks - 1000) * 5
    show tradePrice
  ]
  if(humanAction = 3)[
    set humanAction 0
...
[
  ...
  set currentAsk int((firstInSellQueue / 10000000000000))   ;ask
  set currentBid int((firstInBuyQueue / 10000000000000))    ;bid
  ...
  let orderB int (((sub2Math * 100000) - (currentBid * 100000000000))/ 100000)   ;orderID
  let orderA int (((sub1Math * 100000) - (currentAsk * 100000000000))/ 100000)   ;orderID
  
  let traderB int (aMath - cMath)   ;traderID
  let traderA int (bMath - dMath)   ;traderID
  
  let quantityB array:item arrayQ traderB   ;unfilled order number
  let quantityA array:item arrayQ traderA   ;unfilled order number
...
;      set buyQueue remove firstInBuyQueue buyQueue
      removeQueue "buy" firstInBuyQueue
;      set sellQueue remove firstInSellQueue sellQueue  
      removeQueue "sell" firstInSellQueue
    
      ;if tradeStatus = "Buy" [
         ;set tradePrice currentAsk
      ifelse orderB > orderA[
...
to writetofile  [shr-owned]
  
;  if(AuditTrail = true)[
  if( (AuditTrail = true) and ((length priceReturns) > 0) ) [    
    set tradeExNumber (tradeExNumber + 1)
    let q array:item arrayQ traderNumber
    file-open "OrderBook.csv"
    file-type tradeOrderNumber ;ORDERID
  ...
  if(ticks  > 1000)[
    if((ticks mod 10) = 8)[
     ...
     if(ticks  > 5000)[
       ...
       [
         set currentMA (last movingAverage)
       ]
    ...
    [
     ...
     [  
        ...
        [
          ask players with [ user-name = hubnet-message-source ]
          [
            ifelse(is-number? hubnet-message)[set buypriceticks hubnet-message][carefully [ set buypriceticks read-from-string hubnet-message] [ set buypriceticks 1000 ]]
            if(buypriceticks < 1000)[set buypriceticks 1000]
          ]
        ...
        [
          ask players with [ user-name = hubnet-message-source ]
          [
            ifelse(is-number? hubnet-message)[set sellpriceticks hubnet-message][carefully [ set sellpriceticks read-from-string hubnet-message] [ set sellpriceticks 1000 ]]
            if(sellpriceticks < 1000)[set sellpriceticks 1000]
          ]
        ]
...
to setup-humantraders  ;; player procedure for setup
  ...
  set tradeAccount 0
  set sharesOwned 0
  set totalBought 0     ;num of long positions
  set totalSold 0       ;num of sell positions
  set color brown
  set averageBoughtPrice 0     ;VWAP long
  set averageSoldPrice 0       ;VWAP short
  let dumpTimer 0
  set marketPauseTickCount 0
  set totalCanceled 0
  set buysell "B"
...
to send-student-info ;; player procedure
  ...
  hubnet-send user-name "VWAP Bought" precision averageBoughtPrice 2
  hubnet-send user-name "VWAP Sold" precision averageSoldPrice 2
  hubnet-send user-name "TWAP" precision TWAP 2
  hubnet-send user-name "VWAP" precision VWAP 2
  ;hubnet-send user-name "Account Value" precision tradeAccount 2
end
...
to flashcrash
  ;set stopTriggered 0
  set stopTriggerCounter 0
  set crashPrice (1000 + price / 5)
  set stablePrice price
  set crash true
end

to updateVolume
  if((length minVolumeQueue) = 30)
  [
    set minVolume sum minVolumeQueue
    set minVolumeQueue but-first minVolumeQueue
  ]
  set minVolumeQueue lput volume minVolumeQueue
  
end

to stopcrash
  if(stopTriggerCounter >= 0 and not marketPause)
  [
    set stopTriggerCounter (stopTriggerCounter + 1)
    if(stopTriggerCounter > 600) [
      set stopTriggered 0
      set stopTriggerCounter -1
    ]
  ]
  if(crashPrice > 0 and not crash and  (1000 + price / 5) >= crashPrice)
  [
    print "end"
    set crash FALSE
    set crashTriggered FALSE
  ...
  ;if(crashPrice > 0 and crashTriggered and stopTriggerCounter > 100 and price >= crashPrice)
  ;[
  ;  print "end"
  ;  set crash FALSE
  ;  set crashTriggered FALSE
...
to pause
  if(crash and not marketPause)[
    ;show (crashPrice - (1000 + price / 5)) / crashPrice
    if ((crashPrice - (1000 + price / 5)) / crashPrice > 0.013) [
      set marketPause True
      set stopLoss False
      set crash FALSE
      set crashTriggered FALSE
      set stopMarketMakers FALSE
...
to marketmakerwithdraw
  if(crash) [
    ; withdraw from market
    ifelse(not stopMarketMakers and (currentMA - (1000 + (price / 5))) > 0.24)[
    ;if(not stopMarketMakers and crashTriggered)[
      set stopMarketMakers TRUE
      
      ;set crashTriggered TRUE
      ;print "CRASH"
    ]
    [
      if(stopTriggerCounter > 1000 and not stopMarketMakers)[
        print "end"
        set crash FALSE
        set crashTriggered FALSE
        set crashPrice 0
...
to stoplosswithdraw
  if(crash)[
    if(not stopLoss and (crashPrice - (1000 + (price / 5))) > 0.7)[
      set stopLoss True
    ]
  ]
end
...
to test-print [shr-owned]
  
  ;set priceReturns lput 1 priceReturns
  ...
  file-open "test.csv"
  file-type ", "
  file-type length priceReturns ; file-type last priceReturns
  file-type ", "
  file-type nBidDepth1 
  file-type ", "
  file-type nBidDepth3
...
; add order in buy queue and maintain table by price
to addQueue [buy-sell orderrec ]

  let queue-price int((orderrec / 10000000000000))

  let aMath int (orderrec / 100)
  let sub2Math int((orderrec / 10000000))
  let cMath int (sub2Math * 100000)
  let eMath int (aMath - cMath)
  let value array:item arrayQ eMath
  
  let val 0  
  
  ifelse (buy-sell = "buy") 
  [  
    set buyQueue lput orderrec buyQueue 
    
    ifelse (table:has-key? tblBidDepth queue-price) [
       set val table:get tblBidDepth queue-price
       set val val + value
    ][
       set val value
    ]
    
    table:put tblBidDepth queue-price val
  ][ 
    set sellQueue lput orderrec sellQueue 
    
    ifelse (table:has-key? tblAskDepth queue-price) [
       set val table:get tblAskDepth queue-price
       set val val + value
    ][
       set val value
    ]
    
    table:put tblAskDepth queue-price val
  ]
  
end

; remove order in buy queue and maintain table by price
to removeQueue [buy-sell orderrec]
  ...
  ifelse (buy-sell = "buy")   [  
    set buyQueue remove orderrec buyQueue 
    if (table:has-key? tblBidDepth queue-price) [
      set val table:get tblBidDepth queue-price
      set val val - value
      
      ifelse (val <= 0)    [
        table:remove tblBidDepth queue-price
      ][
        table:put tblBidDepth queue-price  val 
      ]
    ]
  ][ 
    set sellQueue remove orderrec sellQueue 
    if (table:has-key? tblAskDepth queue-price) [
      set val table:get tblAskDepth queue-price
      set val val - value
      
      ifelse (val <= 0)    [
        table:remove tblAskDepth queue-price
      ][
       table:put tblAskDepth queue-price  val 
      ]
    ]
...
to generate-depths
   ...
   ;; get level1 and level 3 on buy side 
   set lst-buy-order  table:keys tblBidDepth
   set lst-buy-order sort-by > lst-buy-order

   ifelse (length lst-buy-order >= 3)
   [
     set nBidDepth1 table:get tblBidDepth item 0 lst-buy-order
     set nBidDepth3 table:get tblBidDepth item 2 lst-buy-order
   ]
   [
     if (length lst-buy-order >= 1)
     [   set nBidDepth1 table:get tblBidDepth item 0 lst-buy-order]
   ]


   ;; get level1 and level 3 on sell side
   set lst-sell-order  table:keys tblAskDepth
   set lst-sell-order sort-by < lst-sell-order

   ifelse (length lst-sell-order >= 3)
   [
     set nAskDepth1 table:get tblAskDepth item 0 lst-sell-order
     set nAskDepth3 table:get tblAskDepth item 2 lst-sell-order
   ]
   [
     if (length lst-sell-order >= 1)
     [   set nAskDepth1 table:get tblAskDepth item 0 lst-sell-order]
   ]
